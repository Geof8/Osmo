...
## Journal rapide
Dernière session: 2026-05-21 — ClinicalStudy : retour au full-bleed sombre 100vh + ajout de la vraie photo de fond `/public/images/study-background.png` (cristaux NAC macro sur fond blanc, fournie par Geoffrey, servie via `next/image` fill+cover+priority). Overlay sombre `linear-gradient(135deg, rgba(0,0,0,.80), rgba(0,0,0,.50))` pour la lisibilité. Carte glassmorphique restaurée (backdrop-blur 12px, bg rgba(255,255,255,.06), border rgba(255,255,255,.12), radius 20px, max 560px) avec eyebrow ambre, headline sobre "L'étude clinique sur la NAC détaillée" (sans "urgentistes"), body NAC/OMS, 3 stats ambre (OMS · +GSH · 40 ans), CTA pilule transparent (radius 50px), disclaimer source. z-index: image 0, overlay 1, contenu 2.
