...
/**
 * Desktop-only intro sequence.
 *
 * Architecture (robust, sticky-free) :
 *  - Outer <div> 300vh : réserve la plage de scroll, défile dans le flow.
 *  - Inner overlay en `position: fixed` (z-60, couvre tout y compris navbar)
 *    affichée uniquement tant que scrollYProgress < 1.
 *  - useScroll offset ["start start", "end start"] → progress 0→1 sur
 *    l'intégralité des 300vh du container. Au moment où progress atteint 1,
 *    on cache l'overlay, et le Hero (placé juste après le container) est
 *    pile au top du viewport → coupe nette, aucun gap blanc possible.
 */
export default function IntroSequence() {
  ...
  // Active step pour progress dots + masquer l'overlay quand on quitte la plage
  useMotionValueEvent(scrollYProgress, "change", (v) => {
    if (v < 0.30) setActiveStep(0);
    else if (v < 0.65) setActiveStep(1);
    else setActiveStep(2);
    // Coupe nette : dès que le container est entièrement scrollé, on cache le fixed
    // (le Hero est pile au top du viewport à cet instant)
